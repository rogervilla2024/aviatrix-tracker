"""
Aviatrix Tracker - Backend API

FastAPI application for serving crash game statistics.
This module provides REST API endpoints for accessing Aviatrix game data
...
ALLOWED_ORIGINS: List[str] = [
    "http://localhost:3000",
    "http://localhost:3001",
    os.getenv("FRONTEND_URL", "https://aviatrixtracker.com"),
    os.getenv("SECONDARY_DOMAIN", "https://www.aviatrixtracker.com"),
]
...
app = FastAPI(
    title="Aviatrix Tracker API",
    description="Real-time statistics API for Aviatrix crash game",
    version="1.0.0",
    docs_url="/docs",
    redoc_url="/redoc"
)
...
async def startup() -> None:
    ...
    """
    logger.info("Starting Aviatrix Tracker API...")
    ...
    try:
        async with db_manager.connect() as db:
            ...
            await db.execute(
                "CREATE INDEX IF NOT EXISTS idx_round_id ON aviatrix_rounds(round_id)"
            )
...
async def get_rounds(
    ...
    try:
        # Get total count
        cursor = await db.execute("SELECT COUNT(*) FROM aviatrix_rounds")
...
async def get_summary(db: aiosqlite.Connection = Depends(get_db)) -> SummaryStats:
    ...
    try:
        ...
        # Calculate median
        cursor = await db.execute("""
            SELECT crash_multiplier FROM aviatrix_rounds
            ORDER BY crash_multiplier
            LIMIT 1 OFFSET (SELECT COUNT(*) FROM aviatrix_rounds) / 2
        """)
...
async def get_distribution(
    ...
    try:
        ...
        for name, condition in DISTRIBUTION_BUCKETS:
            cursor = await db.execute(
                f"SELECT COUNT(*) FROM aviatrix_rounds WHERE crash_multiplier {condition}"
            )
...
async def health() -> HealthResponse:
    ...
    try:
        async with db_manager.connect() as db:
            ...
            cursor = await db.execute(
                "SELECT MAX(created_at) FROM aviatrix_rounds"
            )
...
crash_stats_router = create_crash_game_router(db_manager, "aviatrix")
...
if __name__ == "__main__":
    ...
    logger.info("Starting Aviatrix Tracker API server...")
